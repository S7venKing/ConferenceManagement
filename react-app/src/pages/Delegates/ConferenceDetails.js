...
const ConferenceDetails = () => {
  ...
  const fetchPaymentStatus = async () => {
    try {
      const response = await postRequest(`/registrations/payment-status/${id}`);
  ...
  const uploadFile = async (file, conferenceId) => {
    try {
      const formData = new FormData();
      formData.append("file", file); // Đảm bảo key phải là "file"
      formData.append("conferenceId", conferenceId);

      const response = await api.post("speaker-conference/upload", formData, {
        headers: {
  ...
  // Gọi hàm này khi user chọn file
  const handleFileUpload = (event) => {
    const file = event.target.files[0];
    if (file) {
      uploadFile(file, id); // Thay 1 bằng conferenceId thực tế
    }
  };
    ...
    <div>
      <div className="mx-auto mt-10 p-6 bg-white shadow-lg rounded-xl">
        ...
        <div className="space-y-4">
          ...
          {conference.registrationFee > 0 && paymentStatus.data == null && (
            <button
        ...
        <div className="mt-6 p-6 bg-white shadow-lg rounded-xl">
          ...
          <div className="flex items-center space-x-4">
            ...
            {registrationStatus.status === "Confirmed" && (
              <>
                <p className="my-5">
                  Bạn đã được xét duyệt tham gia với vai trò là{" "}
                  {registrationStatus.conferenceRole}.
                </p>

                {registrationStatus.conferenceRoleId === 2 && (
                  <button
                    className="px-4 py-2 bg-blue-600 text-white rounded-lg shadow-md hover:bg-blue-700 transition"
                    onClick={() => document.getElementById("fileInput").click()}
                  >
                    Đăng tải bài diễn thuyết
                  </button>
                )}
